...
An end to end SQL project: cleaning a messy, real world Glassdoor jobs dataset and using PostgreSQL window functions and aggregation to answer one practical question — **if you're entering the data field today, where should you focus your job search?**
 
That question breaks down into three decisions, and the five queries in this project exist to answer them with evidence rather than guesswork: which title to target, which companies and sectors to prioritize, and which states are actually worth focusing on. Each section below pairs the queries that speak to one of those decisions, and the final section pulls all five findings into a single recommendation.
...
## A note on how much to trust these numbers
 
This dataset is US-only and limited to 672 postings, so every conclusion below should be read as directional, not definitive — a signal worth weighing, not a guarantee. Within that constraint, the bigger risk is smaller subgroups inside the data: many job titles, companies, and even states have very few postings behind them, and a handful of postings producing an "average" can look identical to a group with hundreds of postings behind it. Every query below applies a minimum sample size filter (`HAVING COUNT(*) >= N`) before ranking or averaging, and reports the underlying count (`n`) alongside every result, so each finding can be weighed by how much data actually backs it rather than taken at face value.
...
## Part 1: Which title should I target?
 
Title choice isn't just about which role pays the most on average — it's also about how predictable that pay is. Q1 looks at whether seniority is rewarded within a title family, and Q5 looks at how wide the pay range is within each title. Together they tell you not just what a title pays, but how much variance you should expect.
...
**Title takeaway:** Data Analyst offers a clearer, more predictable path — seniority reliably pays off and the overall pay range is narrower, so outcomes are easier to forecast. Data Scientist pays slightly more on average but with almost triple the internal spread, meaning the title alone tells you much less about what you'll actually earn.
 
---
 
## Part 2: Which companies and sectors should I prioritize?
 
Knowing a sector pays well on average doesn't tell you which employers inside it actually do — and knowing company size correlates with pay doesn't tell you where that trend breaks down. Q2 finds the specific companies worth targeting within each sector, and Q3 checks whether "bigger company" is a reliable shortcut for "better pay."
...
**Company/sector takeaway:** "Bigger is better" holds up to a point (Large outpaces Mid and Small), but breaks down at the very top, so don't default to the biggest-name employer assuming it pays the most. Within Biotech & Pharmaceuticals and Information Technology specifically, there are named companies with enough postings behind them to be genuinely worth targeting rather than guessed at.
 
---
 
## Part 3: Which states are worth focusing my search on?
...
**Location takeaway:** NY, VA, and DC offer the best combined volume-and-pay profile. CA shouldn't be dismissed — it has by far the most opportunities — but its advantage is sheer volume rather than a pay edge.
 
---
 
## Putting it together
 
A fresher targeting the Data Analyst title gets a clearer seniority payoff and a narrower, more predictable pay range than Data Scientist offers. Biotech & Pharmaceuticals and Information Technology stand out as sectors with specific, well-evidenced companies worth prioritizing rather than guessing at, and Large ($1B-$10B revenue) companies hit the sweet spot on pay without the dip seen at the very top of the market. Geographically, NY, VA, and DC offer the strongest combination of pay and opportunity, though CA remains worth considering for its volume alone. None of these are large effects in isolation, and the dataset's size and US-only scope mean they should inform a search strategy rather than dictate one — but stacked together, they point toward a more specific, evidence-backed starting point than "apply broadly and see what happens."
